import { AlertTriangle, Bluetooth, CheckCircle2, Cpu, HardDrive, RefreshCw, Signal, Timer, Wifi } from 'lucide-react'
...
export function OverviewPage() {
  ...
  const stats = [
    ...
    ['模组', status.limitedMode ? '限制模式' : status.modemReady ? '已就绪' : '未就绪', Cpu], ['可用堆内存', `${Math.round(status.freeHeap / 1024)} KB`, HardDrive],
  ...
  return <><SectionTitle eyebrow="OVERVIEW" title="系统概览" description="设备、网络与蜂窝模组的实时运行状态。" action={<Button variant="secondary" onClick={() => refresh()}><RefreshCw size={15} />刷新</Button>} />
    {status.limitedMode && <Card className="notice-card notice-card--warning"><AlertTriangle size={22} /><div><strong>设备正在限制模式下运行</strong><p>未插卡或插入的卡片无效。</p></div></Card>}
