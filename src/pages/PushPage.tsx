...
export function PushPage() {
  const { config, api, notify, refresh } = useDevice(); const [channels, setChannels] = useState<EditableChannel[]>([]); const [testing, setTesting] = useState<number>()
  ...
  async function test(channel: EditableChannel) {
    const body = new URLSearchParams({ index: String(channel.index), type: String(channel.type), name: channel.name, url: channel.url, body: channel.customBody })
    if (channel.key1) body.set('key1', channel.key1)
    if (channel.key2) body.set('key2', channel.key2)
    setTesting(channel.index)
    try {
      const result = await api.post('/api/v1/push/test', body)
      notify(result.message || `通道 ${channel.index + 1} 测试成功`)
    } catch (error) {
      notify(error instanceof Error ? error.message : '推送测试失败', true)
    } finally {
      setTesting(undefined)
    }
  }
  return <><SectionTitle eyebrow="DELIVERY" title="推送通道" description="最多同时启用五个独立通知出口，密钥留空时不会覆盖已有值。" />
    <form onSubmit={save}><div className="channel-list">{channels.map((channel) => <Card key={channel.index} className="channel-card"><div className="channel-header"><div><span className="channel-number">{String(channel.index + 1).padStart(2, '0')}</span><div><h3>{channel.name || `通道 ${channel.index + 1}`}</h3><p>{types[channel.type - 1] || '未知类型'}</p></div></div><div className="channel-controls"><Button type="button" variant="secondary" disabled={testing !== undefined} onClick={() => test(channel)}><FlaskConical size={14} />{testing === channel.index ? '测试中…' : '测试'}</Button><Switch.Root className="switch" checked={channel.enabled} onCheckedChange={(enabled) => update(channel.index, { enabled })}><Switch.Thumb className="switch-thumb" /></Switch.Root></div></div>
